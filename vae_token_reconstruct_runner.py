...
MODEL_VARIATION = "VAE_TOKEN_RECONSTRUCT_"
TOKEN_PLACEHOLDER = -1
...
def main():
    ...
    model = vautoencoder.VariationalAutoEncoder().to(device) #    n_items = 43098
    ...
    for epoch in tqdm(range(args.epoch)):
        ...
        for i, seq  in tqdm(enumerate(train_loader)):
            ...
            # Add special token to last element of the input sequence. 
            seq_with_token[:, -1] = TOKEN_PLACEHOLDER

            outputs, mu, logvar = model(seq_with_token)
            loss = model.loss_function(outputs, seq, mu, logvar)
